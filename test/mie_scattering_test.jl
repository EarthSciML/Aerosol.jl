...
@testitem "Mie function basic evaluation" setup = [MieSetup] tags = [:mie] begin
    ...
    @test Q_scat ≈ Q_ext atol = 1.0e-10
end
...
@testitem "Absorbing vs non-absorbing particles" setup = [MieSetup] tags = [:mie] begin
    ...
    @test Q_abs_nonabs ≈ 0.0 atol = 1.0e-10
...
@testitem "Extinction coefficient (Eq. 15.27)" setup = [MieSetup] tags = [:mie] begin
    ...
    @test 1.0e-6 < expected_b_ext < 1.0e-2  # Reasonable range for m^-1
end
...
@testitem "Reference values from Table 15.2" setup = [MieSetup] tags = [:mie] begin
    ...
    @test Q_scat_water ≈ Q_ext_water atol = 1.0e-6
...
@testitem "RayleighAtmosphere temperature and pressure dependence" setup = [MieSetup] tags = [:mie] begin
    ...
    @test 10.6e-6 / 13.2e-6 ≈ 0.8 rtol = 0.01
end
