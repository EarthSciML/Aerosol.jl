...
@testset "water content" begin
    ...
    ics = Dict([ions[k].m => ics[k] / 1e6 / mw[k] for k in keys(ics)]) # ug/m3 / (1e6 ug/g) / g/mol = mol/m3
    ...
    ws = [Symbolics.value(ModelingToolkit.substitute(w3, [RH => v])) for v in RHs] .* 1e9  # kg / m3 * (1e9 ug/kg) = ug/m3
