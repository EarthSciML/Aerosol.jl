...
u₀ = Dict{Any, Float64}([s => 1.0e-7 for s in states(sys)])
...
plot(
    begin
        plot(
            sol.t,
            sol[sys.a_K2SO4_aqs] ./ sol[sys.a_K2SO4_s],
            label = "a ratio",
            ylim = (0.01, 0.02)
        )
        #plot!(sol.t, sol[sys.a_CaNO32_s] ./ sol[sys.a_CaNO32_aqs], yscale=:log10)
    ...
    plot(
        sol.t,
        sol[sys.a_K2SO4_aqs] ./ sol[sys.a_K2SO4_s] - sol[sys.rxn4.K_eq],
        label = "a ratio - k_eq",
        ylim = (-1e-6, 1e-6)
    ),
...
xx = @test all([ModelingToolkit.check_units(eq) for eq in equations(get_mtk(model))])
...
plot(
    plot(sol[t], sol[f_CaNO32], ylabel = "f_CaNO32", xlabel = "time (s)", label = :none),
    plot(
        sol[t],
    ...
    plot(
        sol[t],
        sol[Ca_aq] * 1e6 * mw[Ca_aq],
        ylabel = "Ca_aq",
        xlabel = "time (s)",
        label = :none
    ),
    plot(sol[t], sol[rxn1₊k_rev], ylabel = "k_rev", xlabel = "time (s)", label = :none),
    plot(sol[t], sol[W] * 1e9, ylabel = "W (ug/m3)", xlabel = "time (s)", label = :none)
)

plot(
    [plot(sol[t], sol[ion], ylabel = ion, xlabel = "time", label = :none)
     for
     ion in ISORROPIA.all_ions]...,
    size = (1000, 800)
)
plot(
    [plot(sol[t], sol[gas], ylabel = gas, xlabel = "time", label = :none)
     for
     gas in ISORROPIA.all_gases]...,
    size = (1000, 800)
)
plot(
    [plot(sol[t], sol[solid], ylabel = solid, xlabel = "time", label = :none)
     for
     solid in ISORROPIA.all_solids]...,
    size = (1000, 800)
)

@testset "Mass balances" begin
    names = [:K, :Ca, :Mg, :NH, :Na, :SO4, :NO3, :Cl, :H]
    for (i, molecs) in enumerate([
        K_molecs,
        Ca_molecs,
        Mg_molecs,
        NH_molecs,
...
plot(
    bar(
        max.((1.e-22), sol.u[end]),
        yscale = :log10,
        legend = :none,
        title = "Not steady state",
        ylim = (1e-10, 1e-6)
    ),
    bar(
        max.((1.e-22), sol_ss.u),
...
defaults = ModelingToolkit.get_defaults(sys)
u₀ = Dict{Any, Float64}([s => 1.0e-20 for s in states(sys)])
u₀[CaNO32_s] = 0.8 / 1e6 / mw[CaNO32_s]
u₀[Ca_aq] = 0.8 / 1e6 / mw[Ca_aq]
...
γaq = substitute(
    ModelingToolkit.subs_constants(
        exp(
        ISORROPIA.logγ₁₂(ISORROPIA.CaNO32_aqs),
    )^(ISORROPIA.CaNO32_aqs.ν_cation + ISORROPIA.CaNO32_aqs.ν_anion),
    ),
    u
)
...
aq_activity2 = substitute(
    ModelingToolkit.subs_constants(ISORROPIA.activity(ISORROPIA.CaNO32_aqs)), u)

@test ModelingToolkit.value(aq_activity1) ≈ ModelingToolkit.value(aq_activity2)

keq = rxn1₊K⁰ * exp(
    -rxn1₊H_group * (rxn1₊T₀ / T - 1) -
    rxn1₊C_group * (1 + log(rxn1₊T₀ / T) - rxn1₊T₀ / T),
)
eq_const = substitute(ModelingToolkit.subs_constants(keq), u)

solid_activity = substitute(
    ModelingToolkit.subs_constants(ISORROPIA.activity(ISORROPIA.CaNO32s)), u)
...
@test isapprox(
    ModelingToolkit.value(aq_activity2) / ModelingToolkit.value(solid_activity) /
    ModelingToolkit.value(eq_const),
    1.0,
    atol = 1e-2
)
...
da_daq = substitute(
    ModelingToolkit.subs_constants(
        expand_derivatives(
        Differential(ISORROPIA.Ca_aq)(ISORROPIA.activity(ISORROPIA.CaNO32_aqs)),
    ),
    ),
    [I_one => 1.0, T₀₂ => 273.15, c_1 => 0.005, u...]
)
...
k_expr = ModelingToolkit.subs_constants(
    ISORROPIA.activity(ISORROPIA.CaNO32_aqs) / ISORROPIA.activity(ISORROPIA.CaNO32s) / keq,
)
...
@test ModelingToolkit.get_unit(
    ISORROPIA.activity(ISORROPIA.CaNO32_aqs) / ISORROPIA.activity(ISORROPIA.CaNO32s) /
    keq,
