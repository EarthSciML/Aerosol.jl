...
@testitem "Kelvin Effect Calculation" tags = [:seinfeld_pandis_ch10] begin
    ...
    R_gas = 8.314
    exponent = 2 * 0.072 * 0.018 / (R_gas * 298.0 * 1000.0 * 50.0e-9)
    S_expected = exp(exponent)

    S = kelvin_saturation_ratio(298.0, 50.0e-9, 0.072, 0.018, 1000.0)
    @test S ≈ S_expected rtol = 1.0e-10

    # Test limiting cases
    # Large particles: Kelvin effect should be negligible (S → 1)
    S_large = kelvin_saturation_ratio(298.0, 1.0e-6, 0.072, 0.018, 1000.0)
    @test S_large ≈ 1.0 atol = 0.002

    S_very_large = kelvin_saturation_ratio(298.0, 1.0e-3, 0.072, 0.018, 1000.0)
    @test S_very_large ≈ 1.0 atol = 1.0e-5

    # Small particles: Kelvin effect should be significant
    S_small = kelvin_saturation_ratio(298.0, 10.0e-9, 0.072, 0.018, 1000.0)
    @test S_small > 1.1

    # Temperature dependence: higher T → lower Kelvin effect
    S_cold = kelvin_saturation_ratio(250.0, 50.0e-9, 0.072, 0.018, 1000.0)
    S_warm = kelvin_saturation_ratio(350.0, 50.0e-9, 0.072, 0.018, 1000.0)
    @test S_cold > S > S_warm

    # Surface tension dependence: higher σ → larger Kelvin effect
    S_low_sigma = kelvin_saturation_ratio(298.0, 50.0e-9, 0.03, 0.018, 1000.0)
    S_high_sigma = kelvin_saturation_ratio(298.0, 50.0e-9, 0.072, 0.018, 1000.0)
    @test S_high_sigma > S_low_sigma

    # Monotonicity: S always >= 1 for physical parameters
    for R_p in [5.0e-9, 10.0e-9, 50.0e-9, 100.0e-9, 1.0e-6]
    ...
    S_benzene = kelvin_saturation_ratio(298.0, 50.0e-9, 0.02821, 0.07811, 879.0)
    @test S_benzene > 1.0
end
...
@testitem "DRH Data Tables" tags = [:seinfeld_pandis_ch10] begin
    ...
    @test SeinfeldPandisCh10.DRH_298[:NH4Cl] ≈ 0.8 rtol = 0.01
    ...
    @test SeinfeldPandisCh10.DRH_298[:NH43HSO42] ≈ 0.69 rtol = 0.01
    @test SeinfeldPandisCh10.DRH_298[:NH4NO3] ≈ 0.618 rtol = 0.01
    @test SeinfeldPandisCh10.DRH_298[:NaHSO4] ≈ 0.52 rtol = 0.01
    @test SeinfeldPandisCh10.DRH_298[:NH4HSO4] ≈ 0.4 rtol = 0.01
...
@testitem "ZSR Water Content" tags = [:seinfeld_pandis_ch10] begin
    ...
    concentrations = Dict(:NH42SO4 => 1.0e-6, :NH4NO3 => 1.0e-6)
    ...
    W_more = zsr_water_content(0.8, Dict(:NH42SO4 => 2.0e-6, :NH4NO3 => 2.0e-6))
    ...
    @test W_zero ≈ 0.0 atol = 1.0e-20

    # Single-component ZSR should equal C/m0(a_w)
    W_single = zsr_water_content(0.8, Dict(:NaCl => 1.0e-6))
    m0_nacl = SeinfeldPandisCh10.binary_molality_nacl(0.8)
    @test W_single ≈ 1.0e-6 / m0_nacl rtol = 1.0e-10

    # Test additivity: W(A+B) = W(A) + W(B) (ZSR mixing rule)
    W_a = zsr_water_content(0.8, Dict(:NH42SO4 => 1.0e-6))
    W_b = zsr_water_content(0.8, Dict(:NH4NO3 => 1.0e-6))
    W_ab = zsr_water_content(0.8, Dict(:NH42SO4 => 1.0e-6, :NH4NO3 => 1.0e-6))
    @test W_ab ≈ W_a + W_b rtol = 1.0e-10
    ...
    for func in [
            SeinfeldPandisCh10.binary_molality_nh42so4,
...
@testitem "NH4NO3 Equilibrium Constants" tags = [:seinfeld_pandis_ch10] begin
    ...
    @test Kp_298 ≈ exp(ln_Kp_expected) rtol = 1.0e-10
    ...
    @test K_AN_298 ≈ 4.0e17 rtol = 1.0e-10  # Exact at reference temperature
    ...
    @test ionic_strength_fraction(1.0, 0.0) ≈ 1.0 atol = 1.0e-10
...
@testitem "Table 10.7 Equilibrium Constants" tags = [:seinfeld_pandis_ch10] begin
    ...
    @test get_equilibrium_constant(:NaCl_HNO3, 298.0) ≈ 3.96 rtol = 1.0e-6
    @test get_equilibrium_constant(:HSO4_dissoc, 298.0) ≈ 1.01e-2 rtol = 1.0e-6
    @test get_equilibrium_constant(:NH3_HNO3_aq, 298.0) ≈ 4.0e17 rtol = 1.0e-6
    @test get_equilibrium_constant(:HCl_dissoc, 298.0) ≈ 2.03e6 rtol = 1.0e-6
    @test get_equilibrium_constant(:NH3_HCl_aq, 298.0) ≈ 2.12e17 rtol = 1.0e-6
    @test get_equilibrium_constant(:Na2SO4_dissoc, 298.0) ≈ 0.48 rtol = 1.0e-6
    @test get_equilibrium_constant(:NH42SO4_dissoc, 298.0) ≈ 1.425 rtol = 1.0e-6
    @test get_equilibrium_constant(:HNO3_dissoc, 298.0) ≈ 3.638e6 rtol = 1.0e-6
    @test get_equilibrium_constant(:NH4Cl_dissoc, 298.0) ≈ 1.039e-16 rtol = 1.0e-6
    @test get_equilibrium_constant(:NH4NO3_solid, 298.0) ≈ 3.35e16 rtol = 1.0e-6
    @test get_equilibrium_constant(:NaCl_dissoc, 298.0) ≈ 37.74 rtol = 1.0e-6
    @test get_equilibrium_constant(:NaHSO4_dissoc, 298.0) ≈ 2.44e4 rtol = 1.0e-6
    @test get_equilibrium_constant(:NaNO3_dissoc, 298.0) ≈ 11.97 rtol = 1.0e-6
    ...
    for rxn in keys(SeinfeldPandisCh10.EQUILIBRIUM_CONSTANTS)
        ...
        @test get_equilibrium_constant(rxn, 298.0) ≈ K_ref rtol = 1.0e-10
    end
