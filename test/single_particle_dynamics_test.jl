...
@testitem "SlipCorrection Knudsen number" setup = [SPDSetup] tags = [:spd] begin
    ...
    Kn_expected = 2 * λ / D_p
    @test sol[csys.Kn] ≈ Kn_expected rtol = 1.0e-6
end
...
@testitem "SettlingVelocity relaxation time from Table 9.4" setup = [SPDSetup] tags = [:spd] begin
    ...
    test_cases = [
        ...
        (1.0e-6, 1.164, 3.6e-6),    # 1 μm
        (1.0e-5, 1.016, 3.14e-4),   # 10 μm
    ]
...
@testitem "SettlingVelocity v_t = τg" setup = [SPDSetup] tags = [:spd] begin
    ...
    g = 9.807
    @test sol[csys.v_t] ≈ sol[csys.τ] * g rtol = 1.0e-6
end
...
@testitem "BrownianDiffusion coefficient from Table 9.5" setup = [SPDSetup] tags = [:spd] begin
    ...
    test_cases = [
        ...
        (1.0e-8, 22.2, 5.38e-8),    # 0.01 μm: D = 5.38e-4 cm²/s = 5.38e-8 m²/s
        (1.0e-7, 2.85, 6.91e-10),   # 0.1 μm: D = 6.91e-6 cm²/s = 6.91e-10 m²/s
        (1.0e-6, 1.164, 2.82e-11),  # 1 μm: D = 2.82e-7 cm²/s = 2.82e-11 m²/s
    ]
...
@testitem "BrownianDiffusion Einstein relation D = BkT" setup = [SPDSetup] tags = [:spd] begin
    ...
    @test sol[csys.D_B] ≈ D_from_BkT rtol = 1.0e-6
end
...
@testitem "ElectricalMobility v_e = B_e * E" setup = [SPDSetup] tags = [:spd] begin
    ...
    @test sol[csys.v_e] ≈ sol[csys.B_e] * E rtol = 1.0e-6
end
...
@testitem "StokesNumber s_p = τU" setup = [SPDSetup] tags = [:spd] begin
    ...
    @test sol[csys.s_p] ≈ sol[csys.τ] * U rtol = 1.0e-6
end
...
@testitem "StokesNumber St = τU/L" setup = [SPDSetup] tags = [:spd] begin
    ...
    @test sol[csys.St] ≈ sol[csys.τ] * U / L rtol = 1.0e-6
end
...
@testitem "AerodynamicDiameter continuum limit" setup = [SPDSetup] tags = [:spd] begin
    ...
    @test sol[csys.D_ca_continuum] ≈ D_ca_expected rtol = 1.0e-6
...
@testitem "SingleParticleDynamics comprehensive validation" setup = [SPDSetup] tags = [:spd] begin
    ...
    # Validate v_t = τ * g
    @test sol[csys.v_t] ≈ sol[csys.τ] * 9.807 rtol = 1.0e-6
...
@testitem "SingleParticleDynamics size dependence" setup = [SPDSetup] tags = [:spd] begin
    ...
    prob1 = NonlinearProblem(csys, Dict(), Dict(csys.D_p => 1.0e-7))
    sol1 = solve(prob1)

    # Large particle (10 μm)
    prob2 = NonlinearProblem(csys, Dict(), Dict(csys.D_p => 1.0e-5))
    sol2 = solve(prob2)
...
@testitem "ParticleMobility B = C_c / (3πμD_p)" setup = [SPDSetup] tags = [:spd] begin
    ...
    @test sol[csys.B] ≈ B_expected rtol = 1.0e-6
end
...
@testitem "ParticleMobility size dependence" setup = [SPDSetup] tags = [:spd] begin
    ...
    prob1 = NonlinearProblem(csys, Dict(), Dict(csys.D_p => 1.0e-7, csys.C_c => 2.85))
    sol1 = solve(prob1)

    # Large particle (10 μm) with lower slip correction
    prob2 = NonlinearProblem(csys, Dict(), Dict(csys.D_p => 1.0e-5, csys.C_c => 1.016))
    sol2 = solve(prob2)
