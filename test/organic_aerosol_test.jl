...
@testitem "ECTracerMethod equations" tags = [:organic_aerosol] begin
    ...
    @test sol[ssys.OC_secondary] ≈ 4.0e-9 rtol = 1.0e-6
...
@testitem "NoninteractingSOA equations" tags = [:organic_aerosol] begin
    ...
    @test sol[ssys.c_total] ≈ c_total_expected rtol = 1.0e-6
    @test sol[ssys.c_aer] ≈ c_aer_expected rtol = 1.0e-6
    @test sol[ssys.c_gas] ≈ c_eq_expected rtol = 1.0e-6
    @test sol[ssys.ΔROG_threshold] ≈ threshold_expected rtol = 1.0e-6
    @test sol[ssys.Y] ≈ c_aer_expected / ΔROG_val rtol = 1.0e-6
    ...
    @test sol2[ssys.c_gas] ≈ c_total_below rtol = 1.0e-6
    @test sol2[ssys.X_p] ≈ 0.0 atol = 1.0e-15
end
...
@testitem "TwoProductSOA equations" tags = [:organic_aerosol] begin
    ...
    @test sol[ssys.Y] ≈ Y_expected rtol = 1.0e-6
end
...
@testitem "LangmuirAdsorption equations" tags = [:organic_aerosol] begin
    ...
    sol = solve(prob)
    @test sol[ssys.V] ≈ 0.5 rtol = 1.0e-6

    # At very high pressure: V → V_m (saturation)
    prob_high = NonlinearProblem(ssys, Dict(ssys.p => 1.0e6, ssys.V => 0.5))
    sol_high = solve(prob_high)
    @test sol_high[ssys.V] ≈ V_m rtol = 1.0e-3
    ...
    @test sol_low[ssys.V] ≈ V_m * b * p_low rtol = 1.0e-3
end
...
@testitem "BETAdsorption equations" tags = [:organic_aerosol] begin
    ...
    sol = solve(prob)
    @test sol[ssys.V] ≈ V_expected rtol = 1.0e-6
    ...
    sol_low = solve(prob_low)
    @test sol_low[ssys.V] ≈ V_low_expected rtol = 1.0e-6
    # At low S, approximately V_m * c_BET * S
    @test sol_low[ssys.V] ≈ V_m * c_BET * S_low rtol = 1.0e-2
end
...
@testitem "FHHAdsorption equations" tags = [:organic_aerosol] begin
    ...
    sol = solve(prob)
    @test sol[ssys.V] ≈ V_expected rtol = 1.0e-6
    @test sol[ssys.V] ≈ 1.0 / log(2.0) rtol = 1.0e-6
    ...
    @test sol_b2[ssys.V] ≈ V_expected_b2 rtol = 1.0e-6
end
...
@testitem "BET reduces to Langmuir" tags = [:organic_aerosol] begin
    ...
    @test bet_sol[sbet.V] ≈ lang_sol[slang.V] rtol = 1.0e-2
end
