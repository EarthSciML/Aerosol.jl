...
@component function Nucleation(; name=:Nucleation)
    ...
    eqs = [
        ...
        J ~ sqrt(2 * σ / (π_val * m_1)) * (v_1 * N_1^2 / S) *
...
@component function WaterProperties(; name=:WaterProperties)
    @constants begin
        ...
        σ_slope = -0.1454e-3, [description = "Surface tension temperature slope", unit = u"N/(m*K)"]
        ...
        A_antoine = 23.1964, [description = "Antoine A coefficient (dimensionless)", unit = u"1"]
        B_antoine = 3816.44, [description = "Antoine B coefficient (dimensionless)", unit = u"1"]
        C_antoine = -46.13, [description = "Antoine C coefficient (dimensionless)", unit = u"1"]
    end

    @parameters begin
        T = 293.0, [description = "Temperature", unit = u"K"]
        T_unit = 1.0, [description = "Unit temperature for conversion (dimensionless)", unit = u"K"]
        Pa_unit = 1.0, [description = "Unit pressure for conversion (dimensionless)", unit = u"Pa"]
    end
