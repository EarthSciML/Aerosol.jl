...
@component function DRHTemperature(; name=:DRHTemperature, salt::Symbol=:NH4NO3)
    ...
    @parameters begin
        ...
        A_sol = sol_params[1], [description = "Solubility coefficient A (dimensionless)", unit = u"1"]
    ...
    @variables begin
        DRH(t), [description = "Deliquescence RH at temperature T (dimensionless)", unit = u"1"]
    end

    eqs = [
        # Eq. 10.72: Full form with A, B, C solubility coefficients
        DRH ~ DRH_298K * exp(ΔH_s / R_gas * (A_sol * (1/T - 1/T_ref) - B_sol * log(T/T_ref) - C_sol * (T - T_ref))),  # Eq. 10.72
    ]
...
# Example
```julia
drh = drh_temperature(280.0, :NH4NO3)
# Returns DRH of NH4NO3 at 280 K
```
"""
function drh_temperature(T::Real, salt::Symbol)
    ...
    # Eq. 10.72: Full form with A, B, C solubility coefficients
    return drh_ref * exp(delta_hs / R_gas * (A * (1/T - 1/T_ref) - B * log(T/T_ref) - C * (T - T_ref)))
end
