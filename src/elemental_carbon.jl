...
"""
@component function ElementalCarbon(; name = :ElementalCarbon)
    @parameters begin
    ...
    @constants begin
        zero = 0, [unit = u"ppb/s", description = "Zero change"]
    end
    eqs = [
        D(EC) ~ zero,
        v_placeholder ~ ρ * d_p * T * P * 1.0e-20, # Meaningless equation to prevent parameters from being removed.
    ]
    return System(
        eqs, t;
        name,
