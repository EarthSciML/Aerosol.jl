...
@component function IsorropiaEquilibrium(; name = :IsorropiaEquilibrium)
    ...
    @constants begin
        ...
        # NH3(g) ↔ NH3(aq) (Table 2: mol/(kg·atm) converted to mol/(kg·Pa))
        ...
        # HCl(g) ↔ H⁺ + Cl⁻ (Table 2: mol²/(kg²·atm) converted to mol²/(kg²·Pa))
        ...
        # HNO3(g) ↔ H⁺ + NO3⁻ (Table 2: mol²/(kg²·atm) converted to mol²/(kg²·Pa))
    ...
    # Units converted to SI (Pa instead of atm):
    #   K1: mol/kg, K21: mol/(kg·Pa), K22: mol/kg, K3/K4: mol²/(kg²·Pa), Kw: mol²/kg²
    # Van't Hoff equation intermediate terms (Eq. 5): K(T) = K₀ × exp(A×(T₀/T - 1) + B×(1 + ln(T₀/T) - T₀/T))
    T0T_ratio = T_0 / T_env  # T₀/T (dimensionless)
    vant_hoff_base = (T0T_ratio - 1.0)  # First term: (T₀/T - 1)
    vant_hoff_cp = (1.0 + log(T0T_ratio) - T0T_ratio)  # Second term: (1 + ln(T₀/T) - T₀/T)
    ...
    eqs = [
        ...
        0 ~ (c_H / (W_w * m_ref)) * (c_SO4 / (W_w * m_ref)) * γ_H2SO4^3 -
            (K1_0 * exp(K1_A * vant_hoff_base + K1_B * vant_hoff_cp) / m_ref) * (c_HSO4 / (W_w * m_ref)) * γ_HHSO4^2,
        ...
        0 ~ (c_NH4 / (W_w * m_ref)) * (c_OH / (W_w * m_ref)) * (γ_NH4Cl / γ_HCl)^2 -
            ((K21_0 * exp(K21_A * vant_hoff_base + K21_B * vant_hoff_cp) * K22_0 * exp(K22_A * vant_hoff_base + K22_B * vant_hoff_cp)) / (m_ref^2)) * RH * (g_NH3 * R_gas_si * T_env / p_ref),
        ...
        0 ~ (c_H / (W_w * m_ref)) * (c_NO3 / (W_w * m_ref)) * γ_HNO3^2 -
            (K4_0 * exp(K4_A * vant_hoff_base + K4_B * vant_hoff_cp) / (m_ref^2)) * (g_HNO3 * R_gas_si * T_env / p_ref),
        ...
        0 ~ (c_H / (W_w * m_ref)) * (c_Cl / (W_w * m_ref)) * γ_HCl^2 -
            (K3_0 * exp(K3_A * vant_hoff_base + K3_B * vant_hoff_cp) / (m_ref^2)) * (g_HCl * R_gas_si * T_env / p_ref),

        # 14. H₂O ↔ H⁺ + OH⁻ (Kw, Table 2)
        # Kw [mol²/kg²] = m_H × m_OH / aᵤ
        # Nondim: m̃_H × m̃_OH = Kw_dimless × aᵤ
        0 ~ (c_H / (W_w * m_ref)) * (c_OH / (W_w * m_ref)) - (Kw_0 * exp(Kw_A * vant_hoff_base + Kw_B * vant_hoff_cp) / (m_ref^2)) * RH,
