...
function deliquescence(t, RH, active_salts, salts, all_ions)
    @parameters metastable = 0 [
        description = "Whether the solution is 'metastable' (i.e. RH is decreasing over time). Value should be 1 for 'true' or 0 for 'false'",
    ]
    metastable = ParentScope(metastable)
    @variables MDRH(t) = 0.3 [
        description = "Minimum deliquescence relative humidity (unitless)",
    ]

    drh_eqs = []
    drh_vars = []
    for s in active_salts
        saltname = nameof(s)
