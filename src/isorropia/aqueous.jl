@mtkmodel Ion begin
    ...
    @parameters begin
        z, [description = "Valence (charge) of the ion (dimensionless)"]
    end
    @variables begin
        #! format: off
...
@mtkmodel Salt begin
    ...
    @constants begin
        ...
        # Salt properties
        drh, [description = "Deliquescence relative humidity at 298.15K (dimensionless)"]
        l_t, [description = "Enthalpy term (-18/1000R L_s m_s) (dimensionless, or K when applicable)"]
        q, [description = "Kusik-Meissner Binary activity parameter (dimensionless)"]
        ν_cation = 1, [description = "Number of moles of cation per mole of salt (dimensionless)"]
        ν_anion = 1, [description = "Number of moles of anion per mole of salt (dimensionless)"]
        z_cation = 1, [description = "Absolute value of the cation charge (dimensionless)"]
        z_anion = 1, [description = "Absolute value of the anion charge (dimensionless)"]

        # Derived constants
        B, [description = "Parameter for Kusik-Meissner binary activity (dimensionless)"]
        zz, [description = "Product of the absolute values of the cation and anion charges (dimensionless)"]
    ...
    @variables begin
        ...
        deliquesced(t), [description = "Whether the salt is deliquesced (1) or not (0) (dimensionless)"]
        W(t), [description = "Aerosol water content in air", unit = u"kg/m^3"]
        X(t), [description = "Parameter for activity coefficient calculation (dimensionless)"]
        Y(t), [description = "Parameter for activity coefficient calculation (dimensionless)"]
        I(t), [description = "Ionic strength", unit = u"mol/kg", guess=1]
        logγ⁰(t), [description = "Log of the mean ionic activity coefficient for the single solute solution (dimensionless)"]
        ...
        logΓ⁰(t), [description = "Parameter for Kusik-Meissner activity coefficient (dimensionless)"]
        logΓ⁺(t), [description = "Parameter for Pitzer activity coefficient (dimensionless)"]
        C(t), [description = "Parameter for Pitzer activity coefficient (dimensionless)"]
        F_cat(t), [description = "Activity contribution from the cation (dimensionless)"]
        F_an(t), [description = "Activity contribution from the anion (dimensionless)"]
        A(t), [description = "Parameter used in Equation 14 (dimensionless)"]
        ...
        loga(t), [description = "Log of the activity of the salt (dimensionless)"]
        #! format: on
    end
    @equations begin
        zz ~ z_cation * z_anion
...
@mtkmodel BinaryMolality begin
    ...
    @constants begin
        k_0, [description = "polynomial fit coefficient (dimensionless)"]
        k_1, [description = "polynomial fit coefficient (dimensionless)"]
        k_2, [description = "polynomial fit coefficient (dimensionless)"]
        k_3, [description = "polynomial fit coefficient (dimensionless)"]
        k_4, [description = "polynomial fit coefficient (dimensionless)"]
        k_5, [description = "polynomial fit coefficient (dimensionless)"]
        k_6, [description = "polynomial fit coefficient (dimensionless)"]
        m_one = 1.0, [unit = u"mol/kg", description = "unit molality"]
    end
    @parameters begin
...
@mtkmodel Aqueous begin
    ...
    @variables begin
        ...
        Aᵧ_term(t), [description = "Debye-Hückel term used in Equation 7 and 8 (dimensionless)"]
    end
    @structural_parameters begin
        q0 = 0.0 # Assume q=0 for salts in Table 4 with no data for q (dimensionless)
    end
    @components begin
        # Cations
        ...
        HHSO4 =
            Salt(z_cation = 1, z_anion = 1, drh = 0.0, l_t = NaN, q = 8.0, T = T, RH = RH)
        H2SO4 = Salt(
            z_cation = 1,
            z_anion = 1,
            drh = 0.0,
        ...
        HSO4_dissociated =
            Salt(z_anion = abs(-2), drh = 0, l_t = NaN, q = 0.0, T = T, logm_guess = 1) # H + SO4
        NH3_dissociated = Salt(drh = 0, l_t = NaN, q = 0.0, T = T, logm_guess = 1) # NH4 + OH
        H2O_dissociated = Salt(drh = 0, l_t = NaN, q = 0.0, T = T, logm_guess = 1) # H + OH

        # # Water content
        # #! format: off
        maw_CaNO32 = BinaryMolality(
            k_0 = 36.356,
            k_1 = -165.66,
            k_2 = 447.46,
            k_3 = -673.55,
