...
@mtkmodel Salt begin
    ...
    @equations begin
        ...
        if is_KHSO4 || is_NH4HSO4 || is_NaHSO4  # From Table 4 footnote b, c & d
            logγ ~
                0.5 * (
                ParentScope(salt1.logγ) + ParentScope(salt2.logγ) -
...
@mtkmodel Aqueous begin
    ...
    @variables begin
        ...
        W_x(t), [description = "Aerosol water content in air", unit = u"kg/m^3", guess = 1.0e-7]
    ...
    @components begin
        ...
        CaCl2 = Salt(
            z_cation = 2, ν_anion = 2, drh = 0.283, l_t = 551.1, q = 2.4, T = T,
            RH = RH, logm_guess = 0, can_precipitate = false
        )
        CaSO4 = Salt(
            z_cation = 2, z_anion = 2, drh = 0.97, l_t = NaN, q = q0, T = T,
            RH = RH, logm_guess = 0, can_precipitate = false
        )
        K2SO4 = Salt(
            ν_cation = 2, z_anion = 2, drh = 0.9751, l_t = 35.6, q = -0.25, T = T,
            RH = RH, logm_guess = 0, can_precipitate = true
        )
        KNO3 = Salt(
            z_cation = 1, z_anion = 1, drh = 0.9248, l_t = NaN, q = -2.33, T = T,
            RH = RH, logm_guess = 0
        )
        KCl = Salt(
            z_cation = 1, z_anion = 1, drh = 0.8426, l_t = 158.9, q = 0.92, T = T,
            RH = RH, logm_guess = 0
        )
        MgSO4 = Salt(
        ...
        MgNO32 = Salt(
            z_cation = 2, ν_anion = 2, drh = 0.54, l_t = 230.2, q = 2.32, T = T,
            RH = RH, logm_guess = 0, can_precipitate = false
        )
        MgCl2 = Salt(
            z_cation = 2, ν_anion = 2, drh = 0.3284, l_t = 42.23, q = 2.9, T = T,
            RH = RH, logm_guess = 0, can_precipitate = false
        )
        NaCl = Salt(
            z_cation = 1, z_anion = 1, drh = 0.7528, l_t = 25.0, q = 2.23, T = T,
            RH = RH, logm_guess = 0
        )
        Na2SO4 = Salt(
            ν_cation = 2, z_anion = 2, drh = 0.93, l_t = 80.0, q = -0.19, T = T,
            RH = RH, logm_guess = 0, can_precipitate = true
        )
        NaNO3 = Salt(
        ...
        NH4Cl = Salt(
            z_cation = 1, z_anion = 1, drh = 0.771, l_t = 239.0, q = 0.82, T = T,
            RH = RH, logm_guess = 0
        )
        HHSO4 = Salt(
            z_cation = 1, z_anion = 1, drh = 0.0, l_t = NaN, q = 8.0, T = T,
            RH = RH
        )
        H2SO4 = Salt(
            z_cation = 1, z_anion = 1, drh = 0.0, l_t = NaN, q = -0.1, T = T,
            RH = RH, logm_guess = 0
        )
        HNO3 = Salt(
            z_cation = 1, z_anion = 1, drh = 0, l_t = NaN, q = 2.6, T = T,
            RH = RH, logm_guess = 0
        )
        HCl = Salt(
            z_cation = 1, z_anion = 1, drh = 0, l_t = NaN, q = 6.0, T = T,
            RH = RH, logm_guess = 0
        )
        KHSO4 = Salt(
            z_cation = 1, z_anion = 1, drh = 0.86, l_t = NaN, q = q0, T = T,
            RH = RH, is_KHSO4 = true, salt1 = HHSO4, salt2 = KCl, salt3 = HCl, logm_guess = 0
        )
        NH4HSO4 = Salt(
            z_cation = 1, z_anion = 1, drh = 0.4, l_t = 384.0, q = q0, T = T,
            RH = RH, is_NH4HSO4 = true, salt1 = HHSO4, salt2 = NH4Cl, salt3 = HCl, logm_guess = 0
        )
        NaHSO4 = Salt(
            z_cation = 1, z_anion = 1, drh = 0.52, l_t = -45.0, q = q0, T = T,
            RH = RH, is_NaHSO4 = true, salt1 = HHSO4, salt2 = NaCl, salt3 = HCl, logm_guess = 0
        )
        NH43HSO42 = Salt(
            ν_cation = 3, ν_anion = 2, drh = 0.69, l_t = 186.0, q = q0, T = T,
        ...
        HSO4_dissociated = Salt(
            z_anion = abs(-2), drh = 0, l_t = NaN, q = 0.0, T = T,
            logm_guess = 1
        ) # H + SO4
    ...
    @equations begin
        ...
        I ~
            ...
            sum(
                ...
                exp(H.logm) * H.z^2 + exp(OH.logm) * OH.z^2
        ) * m_one
