...
plot!(R_p_range * 1e9, S_organic, label="Organic (σ=0.030 N/m)", linewidth=2, linestyle=:dash)

# Add reference line at S=1
hline!([1.0], label="", color=:gray, linestyle=:dot)

savefig("kelvin_effect.svg"); nothing # hide
```
...
savefig("drh_temperature.svg"); nothing # hide
```
...
savefig("nh4no3_kp.svg"); nothing # hide
```
...
savefig("nh4no3_drh.svg"); nothing # hide
```
...
# Mixed aerosol
W_mixed = [zsr_water_content(RH, Dict(:NH42SO4 => 0.5e-6, :NH4NO3 => 0.5e-6)) for RH in RH_range]
...
savefig("zsr_water.svg"); nothing # hide
```
...
savefig("equilibrium_constants.svg"); nothing # hide
```
...
savefig("ionic_strength.svg"); nothing # hide
```
